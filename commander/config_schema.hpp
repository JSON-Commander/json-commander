...
namespace commander::config_schema {
  ...
  namespace detail {
    ...
    scalar_type_schema(model::ScalarType type,
    ...
    inline nlohmann::json
    type_spec_schema(const model::TypeSpec &spec,
                     const std::optional<std::vector<std::string>> &choices = std::nullopt) {
      return std::visit(
          [&](const auto &s) -> nlohmann::json {
            ...
            if constexpr (std::is_same_v<T, model::ScalarType>) {
              ...
              return {{"type", "array"}, {"items", scalar_type_schema(s.element)}};
            } else if constexpr (std::is_same_v<T, model::PairType>) {
              return {{"type", "array"},
                      {"prefixItems", {scalar_type_schema(s.first), scalar_type_schema(s.second)}},
                      {"items", false},
                      {"minItems", 2},
                      {"maxItems", 2}};
      ...
      return std::visit(
          [](const auto &a) -> std::pair<std::string, nlohmann::json> {
            ...
            if constexpr (std::is_same_v<T, model::Flag>) {
              std::string dest = a.dest.value_or(arg::detail::resolve_dest(a.names));
    ...
    is_required(const model::Argument &argument) {
      return std::visit(
          [](const auto &a) -> bool {
            using T = std::decay_t<decltype(a)>;
            if constexpr (std::is_same_v<T, model::Flag> || std::is_same_v<T, model::FlagGroup>) {
              return true;
            } else if constexpr (std::is_same_v<T, model::Option>) {
    ...
    inline nlohmann::json
    generate(const std::vector<model::Argument> &args, const std::string &name) {
  ...
  inline nlohmann::json
  to_config_schema(const model::Root &root, const std::vector<std::string> &command_path) {
    if (command_path.empty()) {
      return to_config_schema(root);
    }
