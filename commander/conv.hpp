...
namespace commander::conv {
  ...
  string_conv() {
    return {
        ...
        [](const nlohmann::json &j) -> std::string { return j.get<std::string>(); },
        "STRING",
    };
  }
  ...
  int_conv() {
    return {
        [](const std::string &s) -> nlohmann::json {
          if (s.empty()) {
            throw Error("expected integer, got empty string");
          }
          int value = 0;
        ...
        [](const nlohmann::json &j) -> std::string { return std::to_string(j.get<int>()); },
        "INT",
    };
  }
  ...
  float_conv() {
    return {
        [](const std::string &s) -> nlohmann::json {
          if (s.empty()) {
            throw Error("expected float, got empty string");
          }
          std::size_t pos = 0;
          double value = 0;
          try {
  ...
  bool_conv() {
    return {
        ...
        [](const nlohmann::json &j) -> std::string { return j.get<bool>() ? "true" : "false"; },
        "BOOL",
    };
  }
  ...
  inline Converter
  pair_conv(Converter first, Converter second, const std::string &separator = ",") {
    return {
        [first, second, separator](const std::string &s) -> nlohmann::json {
          auto pos = s.find(separator);
          if (pos == std::string::npos) {
            throw Error("expected pair separated by '" + separator + "', got '" + s + "'");
          }
  ...
  inline Converter
  triple_conv(Converter first,
              Converter second,
              Converter third,
              const std::string &separator = ",") {
    return {
        [first, second, third, separator](const std::string &s) -> nlohmann::json {
          auto pos1 = s.find(separator);
          if (pos1 == std::string::npos) {
            throw Error("expected triple separated by '" + separator + "', got '" + s + "'");
          }
          ...
          auto b = s.substr(pos1 + separator.size(), pos2 - pos1 - separator.size());
        ...
        [first, second, third, separator](const nlohmann::json &j) -> std::string {
          return first.format(j[0]) + separator + second.format(j[1]) + separator +
                 third.format(j[2]);
        },
  ...
  inline Converter
  make(const model::TypeSpec &spec,
