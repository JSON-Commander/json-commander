...
TEST_CASE("run: parse ok", "[run]") {
  ...
  json captured;
  int rc = json_commander::run(cli, args.argc(), args.argv(), [&](const json &config) {
    captured = config;
    return 0;
  });
...
TEST_CASE("run: callback return value propagated", "[run]") {
  ...
  int rc = json_commander::run(cli, args.argc(), args.argv(), [](const json &) { return 42; });
...
TEST_CASE("run: json string overload parse ok", "[run]") {
  ...
  json captured;
  int rc =
      json_commander::run(std::string(test_cli_json), args.argc(), args.argv(), [&](const json &config) {
        captured = config;
        return 0;
      });
...
TEST_CASE("run: invalid schema returns 1", "[run]") {
  ...
  bool called = false;
  int rc = json_commander::run(std::string("{}"), args.argc(), args.argv(), [&](const json &) {
    called = true;
    return 0;
  });
...
TEST_CASE("run: bad json returns 1", "[run]") {
  ...
  bool called = false;
  int rc = json_commander::run(std::string("not json"), args.argc(), args.argv(), [&](const json &) {
    called = true;
    return 0;
  });
...
TEST_CASE("run_file: loads schema and parses ok", "[run]") {
  ...
  int rc = json_commander::run_file(
      std::filesystem::path(SERVE_SCHEMA), args.argc(), args.argv(), [&](const json &config) {
        captured = config;
        return 0;
      });
