...
TEST_CASE("make(Command) with no args and no subcommands produces empty vectors", "[cmd]") {
...
TEST_CASE("make(Command) with one flag produces one FlagSpec in args", "[cmd]") {
  auto c = make_command("sub");
...
TEST_CASE("make(Command) compiles option with correct converter", "[cmd]") {
  auto c = make_command("sub");
  c.args = std::vector<model::Argument>{make_option({"count"}, model::ScalarType::Int)};
  auto spec = cmd::make(c);
...
TEST_CASE("make(Command) with one subcommand produces one CommandSpec", "[cmd]") {
...
TEST_CASE("Subcommand args are compiled independently from parent", "[cmd]") {
  ...
  child.args = std::vector<model::Argument>{make_option({"count"}, model::ScalarType::Int)};
...
TEST_CASE("make(Root) preserves name and doc", "[cmd]") {
  ...
  REQUIRE(spec.doc == model::DocString{"A great tool", "", "With multiple paragraphs"});
}
...
TEST_CASE("make(Root) with nested subcommands that have args", "[cmd]") {
  ...
  REQUIRE(std::holds_alternative<arg::PositionalSpec>(spec.commands[0].commands[0].args[0]));
}
...
TEST_CASE("Full realistic schema compiles correctly", "[cmd]") {
  ...
  auto &key_pos = std::get<arg::PositionalSpec>(spec.commands[2].commands[1].args[0]);
...
TEST_CASE("make(Command) excludes man envs exits by construction", "[cmd]") {
  ...
  // CommandSpec has no man/envs/exits fields — this compiles and the metadata is dropped
  auto spec = cmd::make(c);
  REQUIRE(spec.name == "sub");
  REQUIRE(spec.args.empty());
  REQUIRE(spec.commands.empty());
