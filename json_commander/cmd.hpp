...
namespace json_commander::cmd {
  ...
  make(const model::Command &cmd) {
    return {
        cmd.name,
        cmd.doc,
        cmd.args.has_value() ? arg::make_all(*cmd.args) : std::vector<arg::ArgSpec>{},
        cmd.commands.has_value() ? make_all(*cmd.commands) : std::vector<CommandSpec>{},
    };
  }
  ...
  make(const model::Root &root) {
    return {
        root.name,
        root.doc,
        root.args.has_value() ? arg::make_all(*root.args) : std::vector<arg::ArgSpec>{},
        root.commands.has_value() ? make_all(*root.commands) : std::vector<CommandSpec>{},
        root.version,
        root.config,
    };
  }
