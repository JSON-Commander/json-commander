...
namespace json_commander::validate {
  ...
  struct Validator {
    std::function<void(const std::string &name, const std::optional<nlohmann::json> &value)> check;
  ...
  all_of(std::vector<Validator> validators) {
    ...
    return {
        [validators = std::move(validators)](const std::string &name,
  ...
  namespace detail {
    ...
    must_exist_for_list(const model::ListType &lt) {
      ...
      if (!scalar_v.has_value()) {
        return std::nullopt;
      }
      auto inner = std::move(*scalar_v);
      return Validator{
          [inner](const std::string &name, const std::optional<nlohmann::json> &value) {
            if (!value.has_value()) {
              return;
            }
    ...
    inline void
    check_element_at(const std::string &name,
    ...
    must_exist_for_pair(const model::PairType &pt) {
      bool has_fs = is_filesystem_type(pt.first) || is_filesystem_type(pt.second);
      if (!has_fs) {
        return std::nullopt;
      }
      return Validator{
          [pt](const std::string &name, const std::optional<nlohmann::json> &value) {
            if (!value.has_value()) {
              return;
            }
    ...
    must_exist_for_triple(const model::TripleType &tt) {
      bool has_fs = is_filesystem_type(tt.first) || is_filesystem_type(tt.second) ||
                    is_filesystem_type(tt.third);
      if (!has_fs) {
        return std::nullopt;
      }
      return Validator{
          [tt](const std::string &name, const std::optional<nlohmann::json> &value) {
            if (!value.has_value()) {
              return;
            }
  ...
  from_option(const model::Option &opt) {
    ...
    if (opt.required.value_or(false)) {
  ...
  from_positional(const model::Positional &pos) {
    ...
    if (pos.required.value_or(false)) {
      parts.push_back(required());
    }
